...
public class createbill extends JFrame implements ActionListener {
	...
	createbill() {
		...
		// Image img=new ImageIcon(this.getClass().getResource("/Button-Add-icon
		// (1).png")).getImage();
		...
		Object[] columns = { "Bill_No","Product Id", "Product Name", "Price", "Quantity" };
			...
			try {
				...
				if (rs.next()) {
					...
					if (c >= f) {
						...
						row[0]=t1.getText();
						row[1] = String.valueOf(cb1.getSelectedItem());
						row[2] = String.valueOf(cb2.getSelectedItem());
						row[3] = t4.getText();
						row[4] = t5.getText();
						model.addRow(row);
					...
					{
						//JOptionPane.showMessageDialog(this,t6.setText(Integer.toString(totalcal)));
						JOptionPane.showMessageDialog(this,"Quantity left : "+d);
					} 
				}
				cn.close();
			}
